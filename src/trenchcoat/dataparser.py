...
import numpy as np
import pandas as pd
from nptdms import ChannelObject, RootObject, TdmsFile, TdmsWriter
...
def printTDMSInfo(path: str, to_log: bool = True) -> str | None:
    ...
    td_struct = subprocess.check_output(["tdmsinfo", "-p", path])  # noqa: S603, S607
    td_str = td_struct.decode("utf-8")
    if not to_log:
        return td_str
    # if the user specified an output file
    with open(to_log if isinstance(to_log, str) else f"{fname}.txt","w") as log:
        log.write(td_str)
    return None if isinstance(to_log, str) else td_str
...
def pandasReadImageCSV(path: str, has_head: bool = True, img_shape: tuple = (464, 348), add_fidx: bool = False, use_chunks: bool = True, **kwargs) -> pd.DataFrame:
    ...
    if len(img_shape) != 2:
        msg = f"Image shape must be 2 elements len(img_shape)=={len(img_shape)}"
    ...
    data = pd.read_csv(
        path,
        delimiter=",",
        skiprows=sr if has_head else 2,  # if the the file has the parameter header, then skip it
        ...
        chunksize=csize,
    )  # chunksize for v. large datasets set to 1 frame
...
def convertCSVToTDMS(ipath: str, opath: str | None = None, **kwargs) -> None:
    ...
    if params:
        if os.path.splitext(ipath)[1] in [".7z", ".zip"]:
            warn("Skipping extracting headers as it can't be retrieved from a compressed file!", stacklevel=2)
        else:
    ...
    data = pd.read_csv(
        ipath,
        delimiter=",",
        skiprows=10 if params else 2,  # if the the file has the parameter header, then skip it
        header=None,  # this is to stop it using the first row of values as the header names
        usecols=list(range(1, img_shape[0] + 1)),  # skip first column
        dtype="float64",  # datatype
        chunksize=int(img_shape[1]),
    )  # chunksize for v. large datasets set to 1 frame
    # create tdms file
    ...
    with TdmsWriter(opath if opath is not None else os.path.splitext(os.path.basename(ipath))[0] + ".tdms") as tdms_writer:
        ...
        for chunk in data:
            tdms_writer.write_segment([root, ChannelObject("flir", "temp", chunk.to_numpy().flatten())])


def convertCSVToHDF5(ipath: str, opath: str | None = None, **kwargs) -> None:
    ...
    with h5py.File(opath if opath is not None else os.path.splitext(os.path.basename(ipath))[0] + ".hdf5", "w") as dest:
        chunk = data.get_chunk(csize).to_numpy().reshape((img_shape[0], img_shape[1], 1))
        # create dataset write first chunk
        data_set = dest.create_dataset(
            "temp",
            data=chunk,
            compression=kwargs.get("cmethod", "gzip"),
            compression_opts=kwargs.get("clevel", 9),
            shape=(img_shape[1], img_shape[0], 1),
            maxshape=(img_shape[1], img_shape[0], None),
        )
...
def getCalibrationParams(params_dict: dict) -> dict:
    ...
    return dict(
        filter(
            lambda x: x[0] in ["FLIR:PlanckR2", "FLIR:PlanckR1", "FLIR:PlanckO", "FLIR:PlanckB", "FLIR:PlanckF", "FLIR:ReflectedApparentTemperature"],
            params_dict.items(),
        ),
    )
...
def Tobj2Robj(T: np.ndarray | float, R2: float, R1: float, O: float, B: float, F: float) -> np.ndarray | float:  # noqa: E741, N803, PLR0913
    """Convert object temperature to raw object value."""
    # reverse calculation
    # from https://exiftool.org/forum/index.php?topic=4898.msg23972#msg23972
    exp_BT = np.exp(B / T)  # noqa: N806
    # numerator
...
def Robj2Tobj(Robj:np.ndarray|float, R2:float, R1:float, B:float, F:float, O:float) -> np.ndarray|float:  # noqa: E741, N803, PLR0913
...
def Tref2Rref(Tref:np.ndarray|float, R2:float, R1:float, B:float, F:float, O:float)-> np.ndarray|float:  # noqa: E741, N803, PLR0913
...
def Rref2Tref(Rref:np.ndarray|float, R2:float, R1:float, B:float, F:float, O:float) -> np.ndarray|float:  # noqa: E741, N803, PLR0913
...
def Raw2Robj(S:float, E:float, Rref:np.ndarray|float) -> np.ndarray|float:  # noqa: N803
    """Convert Raw FLIR value to Raw value for object."""
    return (S - (1 - E) * Rref) / E


def Robj2Raw(Robj:float|np.ndarray, E:float, Rref:np.ndarray|float) -> float|np.ndarray:  # noqa: N803
    """Convert raw value for object to raw FLIR value."""
    return (Robj * E) + (1 - E) * Rref


def changeE(temp:np.ndarray|float, Eold:float, Enew:float, R2:float, R1:float, O:float, B:float, F:float, Tref:np.ndarray|float) -> np.ndarray|float:  # noqa: E741, N803, PLR0913
    ...
    Rref = Tref2Rref(Tref, R2, R2, B, F, O)  # noqa: N806
    # convert temperature to raw object value
    Robj = Tobj2Robj(temp, R2, R1, O, B, F)  # noqa: N806
    # convert raw object to raw values
    raw = Robj2Raw(Robj, Eold, Rref)
    # forward calculate to get new raw object under new emissivity
    Robj = Raw2Robj(raw, Enew, Rref)  # noqa: N806
    # convert new to temperature
    return Robj2Tobj(Robj, R2, R1, B, F, O)


def changeEmissivity(temp:np.ndarray, Eold:float, Enew:float, frame_first:bool=False, **params_dict) -> np.ndarray:  # noqa: C901, N803
    ...
    try:
        R2 = params_dict["FLIR:PlanckR2"]  # noqa: N806
    except KeyError:
        R2 = params_dict["R2"]  # noqa: N806

    try:
        R1 = params_dict["FLIR:PlanckR1"]  # noqa: N806
    except KeyError:
        R1 = params_dict["R1"]  # noqa: N806

    try:
        O = params_dict["FLIR:PlanckO"]  # noqa: E741, N806
    except KeyError:
        O = params_dict["O"]  # noqa: E741, N806

    try:
        B = params_dict["FLIR:PlanckB"]  # noqa: N806
    except KeyError:
        B = params_dict["B"]  # noqa: N806

    try:
        F = params_dict["FLIR:PlanckF"]  # noqa: N806
    except KeyError:
        F = params_dict["F"]  # noqa: N806

    try:
        Tref = params_dict["FLIR:ReflectedApparentTemperature"]  # noqa: N806
    except KeyError:
        Tref = params_dict["Tref"]  # noqa: N806
...
def batchConvert(raw:np.ndarray, fn:callable, frame_first:bool=False, **params_dict) -> np.ndarray:  # noqa: C901
    ...
    try:
        ...
        R2 = params_dict.get("R2", 0.0)  # noqa: N806

    try:
        R1 = params_dict["FLIR:PlanckR1"]  # noqa: N806
    except KeyError:
        R1 = params_dict.get("R1", 0.0)  # noqa: N806

    try:
        O = params_dict["FLIR:PlanckO"]  # noqa: E741, N806
    except KeyError:
        O = params_dict.get("O", 0.0)  # noqa: E741, N806

    try:
        B = params_dict["FLIR:PlanckB"]  # noqa: N806
    except KeyError:
        B = params_dict.get("B", 0.0)  # noqa: N806

    try:
        F = params_dict["FLIR:PlanckF"]  # noqa: N806
    except KeyError:
        F = params_dict.get("F", 0.0)  # noqa: N806

    try:
        Tref = params_dict["FLIR:ReflectedApparentTemperature"]  # noqa: N806
    except KeyError:
        Tref = params_dict.get("Tref", 0.0)  # noqa: N806

    try:
        Eold = params_dict["FLIR:Emissivity"]  # noqa: N806
    except KeyError:
        Eold = params_dict.get("Eold", 0.0)  # noqa: N806

    Enew = params_dict["Enew"]  # noqa: N806
...
def temp2raw(temp:np.ndarray, frame_first:bool=True, **params_dict) -> np.ndarray:
...
def raw2temp(raw:np.ndarray, E:float, frame_first:bool=True, **params_dict) -> np.ndarray:  # noqa: N803
...
def loadTDMSData(path:str|Path, inc_time:bool=True) -> dict:
...
class R_pca:
    """Class for performing RPCA on a given matrix."""

    def __init__(self, D:np.ndarray, mu:float|None=None, lmbda:float|None=None) -> None:  # noqa: N803
    ...
    @staticmethod
    def frobenius_norm(M:np.ndarray) -> np.ndarray:  # noqa: D102, N803
        return np.linalg.norm(M, ord="fro")

    @staticmethod
    def shrink(M:np.ndarray, tau:float) -> np.ndarray:  # noqa: D102, N803
        return np.sign(M) * np.maximum((np.abs(M) - tau), np.zeros(M.shape))

    def svd_threshold(self, M:np.ndarray, tau:float) -> np.ndarray:  # noqa: D102, N803
        U, S, V = np.linalg.svd(M, full_matrices=False)  # noqa: N806
        return np.dot(U, np.dot(np.diag(self.shrink(S, tau)), V))

    def fit(self, tol:float|None=None, max_iter:int=1000) -> tuple:
        ...
        err = np.inf
        # matrices
        Sk = np.zeros(self.D.shape)  # noqa: N806
        Yk = np.zeros(self.D.shape)  # noqa: N806
        Lk = np.zeros(self.D.shape)  # noqa: N806
        # tolerance for error
        _tol = tol if tol > 0.0 else 1e-7 * np.linalg.norm(self.D)
        # reset fitted flag
        self._fitted = False
        # this loop implements the principal component pursuit (PCP) algorithm
        # located in the table on page 29 of https://arxiv.org/pdf/0912.3599.pdf
        while (err > _tol) and it < max_iter:
            Lk = self.svd_threshold(self.D - Sk + self.mu_inv * Yk, self.mu_inv)  # this line implements step 3  # noqa: N806
            Sk = self.shrink(self.D - Lk + (self.mu_inv * Yk), self.mu_inv * self.lmbda)  # this line implements step 4  # noqa: N806
            Yk += self.mu * (self.D - Lk - Sk)  # this line implements step 5  # noqa: N806
...
class StackSeqCSV:
    ...
    def __init__(self, path: str) -> None:
        ...
        path = Path(path)
        self._paths = sorted(Path(path.parent).rglob(path.name), key=lambda x: int(os.path.splitext(os.path.basename(x))[0].split("_")[-1].split(" ")[0]))
        self._nf = len(self._paths)
        self._skip = 0
        self._shape = ()

    # est final size of the stacked CSV
    def estSize(self) -> tuple:
        """Estimate the final size of the stacked CSV file."""
        self._skip = 1
        with open(self._paths[0]) as of:
            line = of.readline()
            ...
            self._cols = list(range(1, cols + 1))
            i = 0
            for _ in of:
                i += 1
    ...
    def stack(self, fout: str, convert: bool = True, progress: bool = True) -> None:
        ...
        if convert:
            ...
            np.savez_compressed(fout, data.to_numpy().reshape((-1, 347, 464))[::-1, ...])
            # remove temp CSV to hold stack
            Path(fout_csv).unlink(missing_ok=True)

    def checkFiles(self, progress:bool=True) -> tuple:
        # initialize progress variables
        ci = 0
        if progress:
            th = 1000 if isinstance(progress, bool) and progress else progress
